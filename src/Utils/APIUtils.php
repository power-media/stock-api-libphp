...
class APIUtils
{
     ...
     */
    public static function generateCommonAPIHeaders(CoreConfig $config, $access_token = null)
    {
        $request_id = static::getUUID();
        if ($access_token !== null) {
            $access_token = 'Bearer ' . $access_token;
        }
        $headers = ['headers' => ['x-api-key' => $config->getApiKey(), 'x-product' => $config->getProduct(), 'Authorization' => $access_token, 'x-request-id' => $request_id]];
        return $headers;
    }
    /**
     ...
     */
    public static function downSampleImage($file_path)
    {
     ...
     */
    private static function _calculateResizeParameters($original_width, $original_height)
    {
        $new_dimension = [];
        ...
        if (max($original_width, $original_height) > static::$_longest_side_maximum) {
            ...
            if ($original_width > $original_height) {
                ...
                if ($original_height > static::$_longest_side_downsample_to) {
